...
import { WorldColors, WorldColorsByTheater, WorldLetters, WorldLettersByTheaters, WorldModifiers, WorldPowerLevels, Worlds, WorldStormKingZones, ZoneCategories, ZoneGroups } from '$lib/constants/stw/worldInfo';
...
export async function getWorldInfoData(accessToken?: string) {
...
export function parseWorldInfo(worldInfoData: WorldInfoData) {
  ...
  for (const [theaterId, data] of Object.entries(rawWorldInfo)) {
    for (const [zone, missions] of Object.entries(data.missions)) {
      ...
      for (const mission of missions) {
        ...
        const modifiers = currentAlert?.missionAlertModifiers?.items.map((modifier) => parseModifier(modifier.itemType)) || null;
        ...
        const missionRewards = mission.missionRewards.items
          ...
          .map((item) => {
            ...
            if (
              isEvolutionMaterial(parsedResource.itemType) &&
...
function parseZone(missionGenerator: string): {
  imageUrl: string;
  type: keyof typeof ZoneCategories | null;
} | undefined {
...
function isEvolutionMaterial(key: string) {
  return (
...
export function parseResource({ key, quantity }: { key: string, quantity: number }) {
  const newKey = key
